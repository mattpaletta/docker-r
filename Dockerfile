...
RUN yum remove -y libssh2-devel wget xz-devel pcre-devel zlib-devel bzip2-devel perl && yum clean all -y && rm -rf /var/cache/yum
#RUN echo "install.packages('tidyverse', repos='http://cran.us.r-project.org')" > /install_packages.R && Rscript /install_packages.R && rm /install_packages.R
